...
- `LoadedAggregate` can only be created for value-semantic aggregate shapes; pointer aggregates and aggregates whose nested struct / array fields recursively expose pointer / map / slice / func / chan / interface / unsafe-pointer state are rejected with `ErrInvalidAggregate`
